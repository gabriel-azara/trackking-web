...
import { MoreHorizontal, Edit, Trash2, Calendar, Target, Link } from "lucide-react"
...
export function GoalCard({ goal, onEdit, onDelete }: GoalCardProps) {
  ...
  const getDeadlineStatus = () => {
    ...
    if (days <= 7) return { text: `${days} dias`, variant: "secondary" as const, days }
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center space-x-3">
            <div className={cn("w-3 h-3 rounded-full", getColorClass(goal.color))} style={getColorStyle(goal.color)} />
        ...
        {goal.milestones && goal.milestones.length > 0 && (
          <div className="mb-3">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-xs text-muted-foreground">{goal.milestones.length} marcos</span>
            </div>
