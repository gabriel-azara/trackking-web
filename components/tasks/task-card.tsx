...
import { MoreHorizontal, Edit, Trash2, Calendar, Clock, CheckSquare, AlertCircle, Minus, ArrowUp } from "lucide-react"
...
export function TaskCard({ task, onToggle, onEdit, onDelete }: TaskCardProps) {
  ...
  const getDueDateStatus = () => {
    ...
    if (overdue) return { text: "Atrasado", variant: "destructive" as const, days }
    if (days === 0) return { text: "Hoje", variant: "secondary" as const, days }
    if (days === 1) return { text: "Amanhã", variant: "outline" as const, days }
    if (days <= 7) return { text: `${days} dias`, variant: "outline" as const, days }
    return null
  }
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center space-x-3 flex-1">
            ...
            {task.color && (
              <div
                className={cn("w-3 h-3 rounded-full", getColorClass(task.color))}
