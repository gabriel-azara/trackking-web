...
import { Plus, Search, Filter, CheckSquare, Calendar, Clock } from "lucide-react"
import { useAuth } from "@/contexts/auth-context"
import { subscribeToUserTasks, deleteTask, updateTask } from "@/lib/firebase/tasks"
...
export default function TasksPage() {
  ...
  const filterTasks = (tasks: Task[], view: "all" | "today" | "upcoming") => {
    ...
    switch (view) {
      case "today":
        return filtered.filter((task) => task.dueDate === getToday() || task.status === "doing")
      case "upcoming":
        return filtered.filter((task) => task.dueDate && task.dueDate > getToday())
      default:
        return filtered
    }
  }
  ...
  const stats = {
    ...
    today: tasks.filter((t) => t.dueDate === getToday() && t.status !== "done").length,
  }
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="all">Todas ({getTasksByView("all").length})</TabsTrigger>
            <TabsTrigger value="today">Hoje ({getTasksByView("today").length})</TabsTrigger>
            <TabsTrigger value="upcoming">Próximas ({getTasksByView("upcoming").length})</TabsTrigger>
          </TabsList>
            ...
            <TabsContent key={view} value={view} className="mt-6">
              {getTasksByView(view as any).length === 0 ? (
                <div className="text-center py-12">
                  ...
                  {!searchTerm && filterPriority === "all" && filterStatus === "all" && view === "all" && (
                    <Button onClick={() => router.push("/tasks/new")}>
                      <Plus className="h-4 w-4 mr-2" />
